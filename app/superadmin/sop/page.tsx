...
export default function SopPage() {
    ...
    // Now SOPs are global, so Superadmins can manage all of them.
    // If we wanted Team members to view this page, we'd make isReadOnly = !isMasterAdmin.
    // For now, this is in superadmin panel.
    const isReadOnly = !isMasterAdmin;

    useEffect(() => {
        dispatch(fetchSuperAdmins());
        dispatch(fetchAdmins());
        // Fetch all SOPs for the company since it's global now
        dispatch(fetchSopGroups({ category: "sop" }));
    }, [dispatch]);
        ...
        <div className="flex flex-col h-[calc(100vh-140px)] gap-4 overflow-hidden">
            ...
            <div>
                <div className="flex items-center justify-between">
                    ...
                    <Button onClick={() => setEditGroup({})} size="sm">Create Global SOP</Button>
                </div>
                <p className="text-muted-foreground text-sm">Create global SOPs and assign them to specific roles or users.</p>
            </div>
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-w-md">
                <DialogHeader><DialogTitle>{group._id ? "Edit SOP Group" : "Create SOP Group"}</DialogTitle></DialogHeader>
                <div className="py-2 flex flex-col gap-4">
                    <div>
                        <label className="text-xs font-semibold text-muted-foreground mb-1 block">Title</label>
                        <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Onboarding Process" autoFocus />
                    </div>
