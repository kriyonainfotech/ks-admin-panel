    ...
    const isDateSelected = (day: Date) => {
        if (viewMode === "single") {
            ...
            return isWithinInterval(startOfDay(day), {
                ...
                end: startOfOfDay(dateRange.to)
            });
        }
    };

    // Helper to avoid build error
    function startOfOfDay(date: Date) {
        return startOfDay(date);
    }

    return (
        <Card className="border shadow-md w-full overflow-hidden rounded-xl bg-card py-0">
            <div className="border-b flex flex-row items-center justify-between bg-card px-3 sm:px-4 py-2 sm:py-3">
